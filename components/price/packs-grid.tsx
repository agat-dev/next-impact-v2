          ...
          {packs.map((pack, idx) => (
            <Card
              ...
              className={`relative overflow-hidden border-2 transition-all duration-500 hover:scale-105 hover:shadow-2xl ${
                pack.popular
                  ? "border-[#F29F05] bg-white shadow-xl"
                  : "border-[#719ED9]/30 bg-[#D0DCF2]/30 backdrop-blur-sm"
            ...
            >
              ...
              <div className="p-8">
                <div className={`rounded-xl p-4 w-fit mb-6 ${pack.popular ? "bg-[#020F59]" : "bg-[#020F59]"}`}>
                  <div className="h-8 w-8 flex items-center justify-center">
                    <pack.icon />
                  </div>
                </div>

                <Badge className={`mb-3 ${pack.popular ? "bg-[#020F59] text-white" : "bg-[#1F54BF] text-white"}`}>
                  {pack.tagline}
                </Badge>

                <h3 className="text-2xl font-bold mb-3 text-[#020F59]">{pack.name}</h3>

                <p className="mb-6 leading-relaxed text-[#021373]/70">{pack.description}</p>

                <div className="mb-6">
                  <div className={`text-3xl font-bold mb-2 ${pack.popular ? "text-[#1F54BF]" : "text-[#1F54BF]"}`}>
                    {pack.price}
                  </div>
                </div>
                    ...
                    >
                      <div className="flex-shrink-0 mt-0.5">
                        {typeof feature.icon === "function" ? (
                          ...
                          <feature.icon className="h-5 w-5 text-[#1F54BF]" />
                        )}
                      </div>
                      <div>
                        <div className="font-semibold text-[#020F59]">{feature.text}</div>
                        {feature.desc && <div className="text-sm text-[#021373]/70">{feature.desc}</div>}
                      </div>
                    </li>
                  ))}
                </ul>

                <div className={`rounded-lg p-4 mb-6 ${pack.popular ? "bg-[#D0DCF2]" : "bg-[#D0DCF2]/50"}`}>
                  <div className="text-sm font-semibold mb-1 text-[#021373]">Livrable:</div>
                  <div className="text-sm text-[#020F59]">{pack.deliverable}</div>
                </div>
